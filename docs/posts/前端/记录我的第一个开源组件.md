...
title: 记录我的第一个开源组件
date: 2021-10-20
abstract: 在 B 站上找 Vue3 的教学视频时候发现 Young村长 和 Kagol 老师教学如何写一个开源组件库。于是便踊跃参加了，希望通过这个机会提升自己并为开源社区做贡献。本文将讲述我在这段时间中开发骨架屏（Skeleton）组件的思路以及学习 Vue3 的一些总结。
tags:
  - Vue.js
---
...
在 B 站上找 Vue3 的教学视频时候发现 [Young 村长](https://space.bilibili.com/480140591/?spm_id_from=333.999.0.0) 和 Kagol 老师教学如何写一个开源组件库。于是便踊跃参加了，希望通过这个机会提升自己并为开源社区做贡献。本文将讲述我在这段时间中开发骨架屏（Skeleton）组件的思路以及学习 Vue3 的一些总结。
...
动画效果用 CSS 来实现，我参考了[起码课的 CSS 实现骨架屏效果](https://www.bilibili.com/video/BV1Z5411J72V?spm_id_from=333.999.0.0)，这里只写出最关键的动画效果。
...
_PS：由于写的是开源组件库，在涉及到修改公共样式的时候要把样式包裹在组件样式里。_
...
export default defineComponent({
  ...
  setup(props: SkeletonProps, ctx) {
          ...
          {arr.map((item) => {
            return (
              <div
                ...
                style={
                  round ? "border-radius: 1em;" : "" + `width: ${item.width}`
                }
              />
            );
          })}
    ...
    function renderTitle(isVisible, titleWidth, isRound) {
      ...
      return (
        renderTitleWidth(titleWidth) +
        renderBorderRadius(isRound) +
        renderTitleVisibility(isVisible)
      );
    }
    ...
    return () => {
      return (
        <div class={`devui-skeleton ${renderAnimate(props.animate)}`}>
...
在写组件的过程中我学习到的另一大技能便是单元测试，没有了解过的小伙伴我推荐去看[vue3 的测试指南](https://www.bilibili.com/video/BV1po4y1U79s?spm_id_from=333.999.0.0)，能够快速上手。
...
4. [学会使用 Vue JSX，一车老干妈都是你的](https://juejin.cn/post/6846687590704381959)
5. [vue3 的测试指南](https://www.bilibili.com/video/BV1po4y1U79s?spm_id_from=333.999.0.0)
